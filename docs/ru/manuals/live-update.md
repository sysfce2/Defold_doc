...
При бандлинге любой исключенный ресурс будет оставлен вне бандла приложения. Если установить флажок *Publish Live update content*, Defold либо загрузит исключенные ресурсы в Amazon, либо создаст Zip-архив, в зависимости от того, как настроен Live Update (см. выше). Опубликованный контент Live Update по-прежнему содержит `liveupdate.game.dmanifest` с полным списком ресурсов, необходимых для удаленной доставки.

При публикации архивного контента Live Update опция *Strip Live Update Entries from Main Manifest* (`liveupdate.exclude_entries_from_main_manifest`) включена по умолчанию. В этом режиме ресурсы, предназначенные только для Live Update, удаляются из встроенного `game.dmanifest`, что уменьшает размер бандла и потребление памяти во время выполнения. Отключайте эту опцию только если вам нужно устаревшее поведение, при котором исключенные записи остаются во встроенном `game.dmanifest`.

При включенной настройке по умолчанию `collectionproxy.get_resources()` возвращает `{}`, пока соответствующий архив не смонтирован. После монтирования она возвращает хеши ресурсов для этого proxy.
...
::: important
Устаревший сценарий Live Update с загрузкой отдельных ресурсов больше не рекомендуется. Не используйте `collectionproxy.missing_resources()`, устаревшие manifest API (`liveupdate.get_current_manifest()`, `liveupdate.store_resource()`, `liveupdate.store_manifest()`, `liveupdate.store_archive()`, `liveupdate.is_using_liveupdate_data()`) и старые алиасы-помощники `resource.*` (`resource.get_current_manifest()`, `resource.store_resource()`, `resource.store_manifest()`, `resource.store_archive()`, `resource.is_using_liveupdate_data()`) в новых проектах.

В современных проектах следует публиковать архивы, монтировать их через `liveupdate.add_mount()`, управлять ими с помощью `liveupdate.get_mounts()` и `liveupdate.remove_mount()`, а `collectionproxy.get_resources()` использовать только когда нужно проверить исключенный контент для proxy. Устаревшие ключи подписи манифеста больше не участвуют в этом пайплайне: `publickey` и `privatekey` в `liveupdate.settings` считаются устаревшими и не используются, а `game.public.der` больше не создается и не включается в бандл.
:::
